...
class BookingService:
    def __init__(self, booking_repo, train_repo):
    ...
    async def _next_sequence(self, schedule_id, class_type, status):
        return await self.booking_repo.get_next_sequence(
            schedule_id,
            class_type,
            status,
        )
    ...
    async def book_ticket(self, request, user_id):
        train = await self.train_repo.find_train_by_id(request.train_id)
        if not train:
            raise ResourceNotFoundException("Train doesnt exists")

        schedule = await self.booking_repo.find_schedule(
            request.train_id,
            request.journey_date,
        )
        if not schedule:
            raise BookingOperationException(
                "Journey date is not available for this train"
            )

        if request.booking_status == PassengerStatus.CANCELLED:
            raise BookingOperationException("Invalid booking status")
        ...
        available_seats = await self._get_available_seats(
            request.train_id,
            request.journey_date,
            class_type,
        )
        confirmed_available = len(available_seats)
        current_rac = await self.booking_repo.get_queue_count(
            schedule.id,
            class_type,
            PassengerStatus.RAC,
        )
        rac_capacity = await self.booking_repo.get_total_rac_capacity(
            request.train_id,
            class_type,
        )

        if request.booking_status == PassengerStatus.CNF:
            if confirmed_available < group_size:
                raise BookingOperationException(
                    "Not enough confirmed seats available for the entire group"
                )
        elif request.booking_status == PassengerStatus.RAC:
            if confirmed_available > 0:
                raise BookingOperationException(
                    "RAC booking is not allowed while confirmed seats are available"
                )
            if current_rac + group_size > rac_capacity:
                raise BookingOperationException(
                    "Not enough RAC capacity available for the entire group"
                )
        elif request.booking_status == PassengerStatus.WL:
            if confirmed_available > 0:
                raise BookingOperationException(
                    "Waitlist booking is not allowed while confirmed seats are available"
                )
            if current_rac < rac_capacity:
                raise BookingOperationException(
                    "Waitlist booking is not allowed while RAC capacity is available"
                )
        else:
            raise BookingOperationException("Invalid booking status")
        ...
        if request.booking_status == PassengerStatus.CNF:
            ...
            for passenger_id in passenger_ids:
                ...
                next_sequence += 1
            message = "Ticket booked in waitlist"
    ...
    async def cancel_ticket(self, booking_id, user_id):
        booking = await self.validate_booking_request(booking_id, user_id)
        ...
        if status == PassengerStatus.CNF:
            ...
            await self.cancel_waiting_ticket(booking)
    ...
    async def cancel_confirm_ticket(self, booking):
        ...
        confirmed_seats_no = [
            passenger.seat_id for passenger in booking.passengers
        ]

        total_confirmed_seats = len(confirmed_seats_no)

        rac_passengers = await self.booking_repo.find_top_k_status_passengers(
            booking,
        ...
        waiting_passengers = await self.booking_repo.find_top_k_status_passengers(
            booking,
    ...
    async def cancel_waiting_ticket(self, booking):
        pass

    async def promote_waiting_passengers(self, booking):
        rac_passengers = booking.passengers

        waiting_passenger = await self.booking_repo.find_top_k_status_passengers(
            booking,
            len(rac_passengers),
            PassengerStatus.WL
        )
    ...
    async def validate_booking_request(self, booking_id, user_id):
    ...
    async def validate_journey(self,train_id,journey_date):
        journey=await self.booking_repo.find_schedule(train_id, journey_date)
        if not journey:
    ...
    async def get_queue(self, train_id, journey_date, class_type, status):
        ...
        return await self.booking_repo.get_active_queue(
            train_id,
            journey_date,
            class_type,
            status,
    ...
    async def get_availability(self, train_id, journey_date, class_type):
        ...
        rac = await self.booking_repo.get_queue_count(
            schedule.id,
            class_type,
            PassengerStatus.RAC,
        )
        waitlist = await self.booking_repo.get_queue_count(
            schedule.id,
            class_type,
            PassengerStatus.WL,
        )
